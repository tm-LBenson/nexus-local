...
8. Persist per-file scan entries with imported/skipped/failed outcome, reason, message, size, and imported document ID when available.
9. Transition source `scanning -> active`, stamp `last_scan_at`, and persist imported/skipped/failed counts.
10. Transition the source scan job `running -> succeeded`.
