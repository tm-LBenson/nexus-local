...
`GET /v1/data-sources?tenant_id=<tenant_id>` lists managed knowledge sources for a tenant. `GET /v1/data-sources/<source_id>?tenant_id=<tenant_id>` returns the source plus recent related source-scan jobs and recent `scan_entries` with file path, outcome, reason, message, size, and imported document ID when available. `POST /v1/data-sources` creates a source record with type, name, root path, owner, status, timestamps, and last-scan imported/skipped/failed counts. `PATCH /v1/data-sources/<source_id>` updates source metadata. `POST /v1/data-sources/<source_id>/scan?tenant_id=<tenant_id>` queues a `source_scan` job for that source and rejects duplicate active scans. The worker walks the source path from its own machine/container, uploads supported files, skips unsupported or unsafe files, records per-file outcomes, and queues normal `document_ingestion` jobs. `DELETE /v1/data-sources/<source_id>?tenant_id=<tenant_id>` archives the source record without deleting documents yet. This is the contract folder, synced drive, network share, export, and connector scan workers attach to.
