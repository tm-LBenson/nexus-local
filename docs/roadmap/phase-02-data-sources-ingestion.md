...
| 2.26 | Plan-guided first scan safety. | First scans prompt when no plan exists or when the current plan is large, skip-heavy, or has failures; failed current plans block scans until replanned. |
| 2.27 | Paged import plan review. | Source plans retain a bounded review sample and the Library detail view lets operators filter and page through planned imports, skips, and failures before scanning. |
| 2.28 | Durable source scan run summaries. | Terminal source scan jobs retain start, finish, duration, and outcome counts; Library shows the latest run with active progress and post-run timing. |
| 2.29 | Source scan cancellation. | Operators can cancel an active source scan from Library; queued/running jobs transition to canceled and the worker stops cooperatively between files. |
...
| 2.34 | Source saved views. | Operators can apply built-in source views and save workspace-scoped custom filters for repeat triage workflows. |
| 2.35 | Filtered source recovery actions. | Operators can retry failed source documents or reindex documents across the currently filtered source list. |
...
| 2.40 | Source management and import permissions review. | Backend source routes use explicit source permissions and the UI keeps read-only users out of upload, source edit, and import actions. |
...
- Done: Slice 2.40 source/import permission split backed by trusted-header authorization tests and read-only-aware UI controls.
- Next: Direct connector workers or Phase 3 retrieval evaluation fixtures.
