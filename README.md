...
Source payloads include `include_patterns`, `exclude_patterns`, `scan_interval_minutes`, and `next_scan_at`. Source detail responses include recent source-scan jobs, a `scan_summary` for the newest scan entries, `scan_entries_page` metadata, and per-file scan entries. `GET /v1/data-sources/{id}` accepts `scan_entry_limit`, `scan_entry_offset`, and `scan_entry_outcome`; `GET /v1/data-sources/{id}/scan-entries.csv` exports scan entries and accepts `outcome`. Source scan jobs import supported files from managed source roots, apply source include/exclude patterns, skip unsupported/unsafe/unchanged files, replace prior documents for changed source paths, delete prior documents for missing source paths or paths removed by the current source filters, queue normal document ingestion jobs for each imported file, and report last-scan imported/skipped/failed counts on the source. The worker queues due scheduled scans when `next_scan_at` is reached and advances the next due time after queuing or completing scans. Source reindex requests queue fresh ingestion jobs for currently active documents referenced by the latest source scan entries while skipping documents that are deleted, missing, or already queued.
