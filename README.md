...
Source detail responses include recent source-scan jobs and per-file scan entries. Source scan jobs import supported files from managed source roots, skip unsupported/unsafe/unchanged files, replace prior documents for changed source paths, queue normal document ingestion jobs for each imported file, and report last-scan imported/skipped/failed counts on the source.
